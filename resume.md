...
의료 LLM(생성형 의무기록)의 학습·양자화·**온프레미스 서빙**을 맡았다 — **동시접속 100명 이상, 병원 폐쇄망 내부 배포**(클라우드 사용 불가).
...
- **AI 에이전트:** AI 코딩 에이전트(Claude, GPT/Codex) 적극 활용 — 스펙·아키텍처·리뷰·검증은 직접, 대규모 구현은 에이전트로 주도. (퍼즐에이아이 초기/중기 코드, 캐글 솔루션, 데이터 라벨링·모델 학습은 전부 직접 했다.)
